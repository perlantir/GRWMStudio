...
final class LooksTests: XCTestCase {
    func testLookPresetListMatchesAvailableCreatorLooks() {
        XCTAssertEqual(
            Looks.all.map(\.name),
            [
                "Sunday Best",
        ...
        XCTAssertEqual(Looks.all.map(\.isPro), [false, false])
    }
    ...
    func testCurrentManifestAvailabilityForLookCards() {
        ...
        XCTAssertTrue(Looks.all.allSatisfy { EffectCatalog.shared.containsSync(effectID: $0.effectID) })
    }
